...
//    laik_set_loglevel(LAIK_LL_Info);
    ...
    unsigned char hashSample[SHA_DIGEST_LENGTH];
    ...
    unsigned char hashOriginal[SHA_DIGEST_LENGTH];
    ...
    Laik_Checkpoint checkpoint = laik_checkpoint_create(inst, space, originalData, NULL);
    ...
    if(memcmp(hashOriginal, hashBackup, SHA_DIGEST_LENGTH) == 0) {
    ...
    unsigned char hashBackupOriginalPartitioning[SHA_DIGEST_LENGTH];
    test_hexHash("Checkpoint data using restored partitioning", backupBase, length, hashBackupOriginalPartitioning);
    if(memcmp(hashOriginal, hashBackupOriginalPartitioning, SHA_DIGEST_LENGTH) != 0) {
