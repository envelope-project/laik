...
static
void laik_shmem_addMpiReq(Laik_ActionSeq* as, int round,
...
static
void laik_shmem_addMpiIrecv(Laik_ActionSeq* as, int round,
...
static
void laik_shmem_addMpiIsend(Laik_ActionSeq* as, int round,
...
static
void laik_shmem_addMpiWait(Laik_ActionSeq* as, int round, int req_id)
{
    Laik_A_MpiWait* a;
...
bool laik_shmem_asyncSendRecv(Laik_ActionSeq* as)
{
    ...
    laik_shmem_addMpiReq(as, 0, count, buf);
    ...
    for(unsigned int i = 0; i < as->actionCount; i++, a = nextAction(a)) {
        ...
        case LAIK_AT_BufSend: {
            ...
            laik_shmem_addMpiIsend(as, a->round + 1,
                                 aa->buf, aa->count, aa->to_rank, req_id);
            laik_shmem_addMpiWait(as, maxround + 2, req_id);
            req_id++;
            break;
        }
        ...
        case LAIK_AT_BufRecv: {
            ...
            laik_shmem_addMpiIrecv(as, 0,
                                 aa->buf, aa->count, aa->from_rank, req_id);
            laik_shmem_addMpiWait(as, a->round + 1, req_id);
            req_id++;
            break;
        }
...
Laik_Instance* laik_init_shmem(int* argc, char*** argv)
{
    ...
    if (argc) {
        ...
        if (err != MPI_SUCCESS) laik_shmem_panic(err);
        d->didInit = true;
    }
...
static
MPIData* shmemData(Laik_Instance* i)
{
    return (MPIData*) i->backend_data;
}

static
MPIGroupData* shmemGroupData(Laik_Group* g)
{
...
void laik_shmem_finalize(Laik_Instance* inst)
{
    ...
    if (shmemData(mpi_instance)->didInit) {
...
static
void laik_shmem_updateGroup(Laik_Group* g)
{
...
static
void laik_shmem_exec_packAndSend(Laik_Mapping* map, Laik_Range* range,
...
static
void laik_shmem_exec_recvAndUnpack(Laik_Mapping* map, Laik_Range* range,
...
static
void laik_shmem_exec_reduce(Laik_TransitionContext* tc, Laik_BackendAction* a,
...
static
void laik_shmem_exec_groupReduce(Laik_TransitionContext* tc,
...
void laik_shmem_exec(Laik_ActionSeq* as)
{
    ...
    int tag = 1;
    MPIGroupData* gd = shmemGroupData(tc->transition->group);
    assert(gd);
    ...
    for(unsigned int i = 0; i < as->actionCount; i++, a = nextAction(a)) {
        ...
        case LAIK_AT_Reduce:
            laik_shmem_exec_reduce(tc, ba, dataType, comm);
            break;
...
static
void laik_shmem_aseq_calc_stats(Laik_ActionSeq* as)
{
    unsigned int count;
    Laik_TransitionContext* tc = as->context[0];
    int current_tid = 0;
...
void laik_shmem_prepare(Laik_ActionSeq* as)
{
    ...
    if (mpi_async) {
        changed = laik_shmem_asyncSendRecv(as);
...
static void laik_shmem_sync(Laik_KVStore* kvs)
{
    ...
    MPI_Comm comm = shmemData(mpi_instance)->comm;
